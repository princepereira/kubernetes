...
func TestClassifyLBError(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "hcn NetworkNotFoundError returns network_not_found",
			err:      hcn.NetworkNotFoundError{NetworkName: "testnet"},
			expected: lbErrNetworkNotFound,
		},
		{
			name:     "hcn NetworkNotFoundError with ID returns network_not_found",
			err:      hcn.NetworkNotFoundError{NetworkID: "net-123"},
			expected: lbErrNetworkNotFound,
		},
		{
			name:     "hnslib NetworkNotFoundError returns network_not_found",
			err:      hnslib.NetworkNotFoundError{NetworkName: "testnet"},
			expected: lbErrNetworkNotFound,
		},
		{
			name:     "LoadBalancerNotFoundError returns not_found",
			err:      hcn.LoadBalancerNotFoundError{LoadBalancerId: "lb-123"},
			expected: lbErrNotFound,
		},
		{
			name:     "EndpointNotFoundError returns not_found",
			err:      hcn.EndpointNotFoundError{EndpointID: "ep-123"},
			expected: lbErrNotFound,
		},
		{
			name:     "NamespaceNotFoundError returns not_found",
			err:      hcn.NamespaceNotFoundError{NamespaceID: "ns-123"},
			expected: lbErrNotFound,
		},
		{
		...
		{
			// isNetworkNotFoundError uses type assertion (not errors.As),
			// so a wrapped NetworkNotFoundError falls through to IsNotFoundError
			// which does use errors.As.
			name:     "wrapped NetworkNotFoundError returns not_found",
			err:      fmt.Errorf("wrapped: %w", hcn.NetworkNotFoundError{NetworkName: "testnet"}),
			expected: lbErrNotFound,
		},
	}
...
func TestLBMetricEmission(t *testing.T) {
	metrics.RegisterMetrics(proxyconfig.ProxyModeKernelspace)
	...
	tests := []struct {
		...
		{
			name:      "update failure with nodeport and not_found",
			metricVec: metrics.WinKernelLBUpdateFailure,
			ipFamily:  "IPv4",
			lbType:    lbTypeNodePort,
			errType:   lbErrNotFound,
		},
		{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			val, err = testutil.GetCounterMetricValue(tc.metricVec.WithLabelValues(tc.ipFamily, string(lbTypeHealthCheck), string(lbErrInvalidData)))
			assert.NoError(t, err)
