...
var (
	loadbalancerGuids = []string{emptyLbID, loadbalancerGuid1, loadbalancerGuid2, "LBID-3", "LBID-4", "LBID-5", "LBID-6", "LBID-7", "LBID-8"}
)
...
func testDualStackService(t *testing.T, enableDsr bool, dualStackFamilyPolicy v1.IPFamilyPolicy, trafficPolicy v1.ServiceExternalTrafficPolicy) {
	...
	assert.True(t, ok, "Failed to cast serviceInfo %q", svcPortName.String())
	assert.Equal(t, v4SvcInfo.hnsID, loadbalancerGuid1, "The Hns Loadbalancer Id %v does not match %v. ServicePortName %q", v4SvcInfo.hnsID, loadbalancerGuid1, svcPortName.String())
	assert.Equal(t, len(v4SvcInfo.loadBalancerIngressIPs), 2, "Expected 2 LoadBalancer Ingress IPs, found %d", len(v4SvcInfo.loadBalancerIngressIPs))
	assert.Equal(t, v4SvcInfo.loadBalancerIngressIPs[0].ip, v4LbIP, "LoadBalancer Ingress IP 0 should be %s", v4LbIP)
	assert.Equal(t, v4SvcInfo.loadBalancerIngressIPs[1].ip, v6LbIP, "LoadBalancer Ingress IP 1 should be %s", v6LbIP)
	assert.Equal(t, v4SvcInfo.nodePorthnsID, loadbalancerGuids[2], "The Hns NodePort Loadbalancer Id %v does not match %v. ServicePortName %q", v4SvcInfo.nodePorthnsID, loadbalancerGuids[1], svcPortName.String())
	assert.Equal(t, v4SvcInfo.loadBalancerIngressIPs[0].hnsID, loadbalancerGuids[3], "The Hns Loadbalancer Ingress Id %v does not match %v. ServicePortName %q", v4SvcInfo.loadBalancerIngressIPs[0].hnsID, loadbalancerGuids[2], svcPortName.String())
	assert.Equal(t, v4SvcInfo.loadBalancerIngressIPs[1].hnsID, emptyLbID, "The Hns Loadbalancer Ingress Id %v does not match empty for IPv6 LB. ServicePortName %q", v4SvcInfo.loadBalancerIngressIPs[1].hnsID, svcPortName.String())
	assert.Equal(t, v4SvcInfo.loadBalancerIngressIPs[0].healthCheckHnsID, loadbalancerGuids[4], "The Hns Loadbalancer HealthCheck Id %v does not match %v. ServicePortName %q", v4SvcInfo.loadBalancerIngressIPs[0].healthCheckHnsID, loadbalancerGuids[3], svcPortName.String())
	...
	assert.True(t, ok, "Failed to cast serviceInfo %q", svcPortName.String())
	assert.Equal(t, v6SvcInfo.hnsID, loadbalancerGuids[5], "The Hns Loadbalancer Id %v does not match %v. ServicePortName %q", v6SvcInfo.hnsID, loadbalancerGuids[5], svcPortName.String())
	assert.Equal(t, len(v6SvcInfo.loadBalancerIngressIPs), 2, "Expected 2 LoadBalancer Ingress IPs, found %d", len(v6SvcInfo.loadBalancerIngressIPs))
	assert.Equal(t, v6SvcInfo.loadBalancerIngressIPs[0].ip, v4LbIP, "LoadBalancer Ingress IP 0 should be %s", v4LbIP)
	assert.Equal(t, v6SvcInfo.loadBalancerIngressIPs[1].ip, v6LbIP, "LoadBalancer Ingress IP 1 should be %s", v6LbIP)
	assert.Equal(t, v6SvcInfo.nodePorthnsID, loadbalancerGuids[6], "The Hns NodePort Loadbalancer Id %v does not match %v. ServicePortName %q", v6SvcInfo.nodePorthnsID, loadbalancerGuids[6], svcPortName.String())
	assert.Equal(t, v6SvcInfo.loadBalancerIngressIPs[0].hnsID, emptyLbID, "The Hns Loadbalancer Ingress Id %v does not match empty for IPv4 LB. ServicePortName %q", v6SvcInfo.loadBalancerIngressIPs[0].hnsID, svcPortName.String())
	assert.Equal(t, v6SvcInfo.loadBalancerIngressIPs[1].hnsID, loadbalancerGuids[7], "The Hns Loadbalancer Ingress Id %v does not match %v. ServicePortName %q", v6SvcInfo.loadBalancerIngressIPs[1].hnsID, loadbalancerGuids[7], svcPortName.String())
	assert.Equal(t, v6SvcInfo.loadBalancerIngressIPs[1].healthCheckHnsID, loadbalancerGuids[8], "The Hns Loadbalancer HealthCheck Id %v does not match %v. ServicePortName %q", v6SvcInfo.loadBalancerIngressIPs[1].healthCheckHnsID, loadbalancerGuids[8], svcPortName.String())
	...
	// Validating NodePort Loadbalancer
	nodePortLb, err = v6Proxier.hcn.GetLoadBalancerByID(v6SvcInfo.nodePorthnsID)
	assert.Equal(t, nil, err, "Failed to fetch NodePort loadbalancer: %v", err)
	assert.NotEqual(t, nil, nodePortLb, "NodePort Loadbalancer object should not be nil")
	assert.Equal(t, baseFlag|hcn.LoadBalancerFlagsIPv6, nodePortLb.Flags, "NodePort Loadbalancer Flags should be %v for IPv6 stack", baseFlag|hcn.LoadBalancerFlagsIPv6)

	// Validating V6 IngressIP Loadbalancer
	ingressLb, err = v6Proxier.hcn.GetLoadBalancerByID(v6SvcInfo.loadBalancerIngressIPs[1].hnsID)
	assert.Equal(t, nil, err, "Failed to fetch IngressIP loadbalancer: %v", err)
	assert.NotEqual(t, nil, ingressLb, "IngressIP Loadbalancer object should not be nil")
