...
type HostNetworkService interface {
	...
	updateLoadBalancer(hnsLbID, sourceVip, vip string, endpoints []endpointsInfo, flags loadBalancerFlags, protocol, internalPort, externalPort uint16, previousLoadBalancers map[loadBalancerIdentifier]*loadBalancerInfo) (*loadBalancerInfo, error)
...
func findLoadBalancerID(endpoints []endpointsInfo, vip string, protocol, internalPort, externalPort uint16) (loadBalancerIdentifier, error) {
	...
	_, err := hashEndpoints(endpoints)
	if err != nil {
		klog.V(2).ErrorS(err, "Error hashing endpoints", "endpoints", endpoints)
		return loadBalancerIdentifier{}, err
	}
	if len(vip) > 0 {
		return loadBalancerIdentifier{protocol: protocol, internalPort: internalPort, externalPort: externalPort, vip: vip}, nil
	}
	return loadBalancerIdentifier{protocol: protocol, internalPort: internalPort, externalPort: externalPort}, nil
}
...
func (hns hns) getAllLoadBalancers() (map[loadBalancerIdentifier]*loadBalancerInfo, error) {
	...
	for _, lb := range lbs {
		...
		// Compute hash from backends (endpoint IDs)
		_, err := hashEndpoints(lb.HostComputeEndpoints)
		if err != nil {
		...
		if len(lb.FrontendVIPs) == 0 {
			...
			id = loadBalancerIdentifier{protocol: uint16(portMap.Protocol), internalPort: portMap.InternalPort, externalPort: portMap.ExternalPort, vip: lb.FrontendVIPs[0]}
		}
...
func (hns hns) updateLoadBalancer(hnsLbID, sourceVip, vip string, endpoints []endpointsInfo, flags loadBalancerFlags, protocol, internalPort, externalPort uint16, previousLoadBalancers map[loadBalancerIdentifier]*loadBalancerInfo) (*loadBalancerInfo, error) {
	var id loadBalancerIdentifier
	vips := []string{}
	if len(vip) > 0 {
		id = loadBalancerIdentifier{protocol: protocol, internalPort: internalPort, externalPort: externalPort, vip: vip}
		vips = append(vips, vip)
	} else {
		id = loadBalancerIdentifier{protocol: protocol, internalPort: internalPort, externalPort: externalPort}
	}

	klog.V(3).InfoS("Updating existing loadbalancer called ===== 2", "hnsLbID", hnsLbID, "endpointCount", len(endpoints))
	...
	klog.V(3).InfoS("Updating existing loadbalancer called ===== 3", "hnsLbID", hnsLbID, "endpointCount", len(endpoints))

	lb, err := loadBalancer.Update()

	if err != nil {
		klog.V(3).InfoS("Updating existing loadbalancer called ===== 4", "hnsLbID", hnsLbID, "error", err)
		return nil, err
	}

	klog.V(1).InfoS("Updated Hns loadbalancer policy resource", "loadBalancer", lb)
	lbInfo := &loadBalancerInfo{
		hnsID: lb.Id,
	}
	// Add to map of load balancers
