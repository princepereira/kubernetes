...
func (hns hns) getAllEndpointsByNetwork(networkName string) (map[string]*(endpointsInfo), error) {
	...
	for _, ep := range endpoints {
		...
		for _, ipConfig := range ep.IpConfigurations {
			if _, ok := endpointInfos[ipConfig.IpAddress]; ok && epInfo.isLocal {
				// If the endpoint is already part of the queried endpoints map and is local, then we should not add it again to the map
				continue
			}
