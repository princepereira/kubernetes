...
func updateLoadBalancer(loadbalancerId string, settings string) (*HostComputeLoadBalancer, error) {
	...
	hr := hcnModifyLoadBalancer(&loadBalancerGuid, settings, &loadBalancerHandle, &resultBuffer)
...
func (loadBalancer *HostComputeLoadBalancer) Update() (*HostComputeLoadBalancer, error) {
	...
	loadBalancer, hcnErr := updateLoadBalancer(loadBalancer.Id, string(jsonString))
	if hcnErr != nil {
		return nil, hcnErr
	}
	return loadBalancer, nil
